...
(define (solve-ax+by=1 a b)
  (define (loop x y r old-x old-y old-r)
    (define (div n m)
      (/ (- n (modulo n m)) m))
    (if (= r 0)
        (cons old-x old-y)
        (let ([quotient (div old-r r)])
          (loop (- old-x (* quotient x))
                (- old-y (* quotient y))
                (- old-r (* quotient r))
                x y r))))
  (loop 0 1 b 1 0 a))
...
(define (RSA-unconvert-list intlist key)
  (let ((n (key-modulus key)))
    (define (unconvert l prev)
      (if (null? l)
          '()
          (let ((deciphered (modulo (+ prev (RSA-transform (car l) key)) n)))
            (cons deciphered (unconvert (cdr l) (car l))))))
    (unconvert intlist 0)))
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  (let ([encr-message
        ...
        [signature
         (RSA-transform (compress (string->intlist message)) sender-private-key)])
    (make-signed-message encr-message signature)))
...
(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  (let ([decr-message
    ...
    (let ([hash-message
           (compress (string->intlist decr-message))]
          [decr-hash
           (RSA-transform (signature cyphertext) sender-public-key)])
      (if (= decr-hash hash-message)
          decr-message
          (display "Sender's signature is incorrect!")))))
