...
Dalej McCarthy opisuje różne funkcje, które można zapisać przy użyciu wcześniej opisanej notacji.
    \begin{itemize}
        ...
        \item $assoc[x;y]$ -- szukanie wartości dla klucza $x$ na liście par
...
\begin{itemize}
    ...
    sytuacji gdzie wyrażenia są przechowywane liniowo.
...
maksymalnie sześć 6-bitowych znaków. Ostatnie słowo wypełnione jest wartościami,
...
\begin{frame}
\frametitle{\textit{Garbage collector}\only<3>{\,\,\,(mark-and-sweep)}}
\begin{enumerate}
    \item \pause Znajdujemy wszystkie rejestry osiągalne z rejestrów 
    bazowych i ustawiamy ich znak na ujemny.
    \item \pause Przeszukujemy obszar pamięci zarezerwowany dla struktur 
    listowych i dodajemy wszystkie rejestry o znaku dodatnim do listy 
    \textit{free-storage}.
\end{enumerate}
\end{frame}
...
\frametitle{Obliczanie funkcji rekurencyjnych}
