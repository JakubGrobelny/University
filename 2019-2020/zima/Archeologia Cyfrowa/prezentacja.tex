...
\begin{enumerate}
    \item \pause Dlaczego LISP był potrzebny?
    \item \pause ,,\textit{Recursive Functions of Symbolic Expressions and 
        Their Computation by Machine}''
    \item \pause Wczesne implementacje
    \item \pause Kolejne dialekty
    \item \pause Lisp-maszyny
\end{enumerate}
\end{frame}
...
\only<4>{\footnotetext[2]{Autorstwa Alfreda Northa Whiteheada i Bertranda 
                            Russella}}
\end{frame}
...
\pause
W 1960 ukazuje się praca Johna McCarthy'ego pt.,,\textit{Recursive Functions 
of Symbolic Expressions And Their Computation by Machine, Part I}''. \pause
\begin{figure}
...
\begin{figure}
    \includegraphics[scale=1.5]{images/church-mccarthy.png}
\end{figure}
...
\frametitle{Listy}
\begin{figure}
    \includegraphics[scale=0.4]{images/lists.png}    
\end{figure}
\end{frame}
...
\frametitle{Listy}
\begin{figure}
    \includegraphics[scale=0.4]{images/lists1.png}    
\end{figure}
\end{frame}
...
\frametitle{Co po LISPie 1 i 1.5?}
\begin{description}
    \item[InterLisp] \pause -- tworzony przez BBN Technologies na PDP-10, 
        później w  Xerox Palo Alto Research Center\pause 
        \begin{itemize}
...
\begin{frame}
    \frametitle{Co po LISPie 1 i 1.5?}
    \begin{figure}
        \includegraphics[scale=1.4]{images/timeline.png}
    \end{figure}
\end{frame}
...
\column{0.4\textwidth}
\begin{figure}
    \includegraphics[scale=0.6]{images/knight-machine.jpg}
    \caption{CONS (\textit{Knight Machine})}
\end{figure}
\end{columns}
\end{frame}
...
\frametitle{Lisp-maszyny}
\begin{itemize}
    \item domknięcia (wymagające podania zmiennych, które powinno 
...
Wady?\pause
\begin{itemize}
    \item W przypadku występowania mutowalnych par, modyfikacja \textit{CDR}a 
...
\begin{frame}
\frametitle{Lisp-maszyny -- kwestie techniczne}
    \begin{itemize}
        \item Początkowo Lisp-maszyny były osobistymi stacjami roboczymi 
              bez obsługi wielu użytkowników \pause
        \item Duży, czarno-bialy wyświetlacz \pause
        \item Klawiatura i mysz \pause
        \item Adapter sieciowy \pause
        \item Dysk twardy i zwykle więcej niż 1 MB pamięci RAM \pause
    \end{itemize}
\end{frame}
...
\frametitle{Lisp maszyny -- kwestie techniczne}
\begin{itemize}
    \item Procesory zwykle nie wykonywały Lispu bezpośrednio 
        ale działały jak maszynay stosowe z instrukcjami 
        zoptymalizowanymi dla LISPu. \pause
    \item Wczesne Lisp-maszyny implementowały instrukcje w mikrokodzie \pause
    \item Hardware'owy type checking i dispatching w runtime (np. jedna 
    operacja dodawania dla wszystkich typów numerycznych).\pause
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Lisp maszyny -- kwestie techniczne}
Przykład skompilowanej funkcji (mikroprocesor Ivory od Symbolics):
\pause
\begin{figure}
    \includegraphics[scale=0.35]{images/compile-lisp.png}
\end{figure}
\pause
\begin{figure}
    \includegraphics[scale=0.35]{images/compile-assembly.png}
\end{figure}
\end{frame}
